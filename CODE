...
st.set_page_config(page_title="Auto Correct Speller")
...
if st.button("Spell Check"):
    if text=="":
        st.text("Please Enter Some Word")
    else:
        tmp_corrections = get_correlations(text,prob_of_occurr,vocab,2)
        for i, word_prob in enumerate(tmp_corrections):
            s+=f"word {i}: {word_prob[0]}, probability {word_prob[1]:.6f}\n"
st.text_area("Possible Words",s)
st.select_slider('How Much Satisfied With Spellings ?', ['Bad','Average', 'Good', 'Excellent'])
...
def add_bg_from_url():
    st.markdown(
         ...
         <style>
         .stApp {{
             background-image: url("https://myticor.com/wp-content/uploads/2017/04/blue-bg.jpg");
